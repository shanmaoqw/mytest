...
public class ErpInterceptor implements Interceptor {
    ...
    public void intercept(Invocation invocation) {
        try {
            ...
            String url = ((HttpServletRequest)controller.getRequest()).getRequestURI();
            String token = controller.getHeader("Admin-Token") != null ? controller.getHeader("Admin-Token") : controller.getCookie("Admin-Token", "");
			/*
			 * if ( !Redis.use().exists(token) && !(url !=null &&
			 * url.indexOf("swagger")!=-1||url.indexOf("api-docs")!=-1) //&& !(xx !=null &&
			 * xx.equals("/swagger/api")) ) { controller.renderJson(R.error(302, "请先登录！"));
			 * return; }
			 */
