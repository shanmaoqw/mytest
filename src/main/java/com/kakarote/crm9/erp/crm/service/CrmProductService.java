...
public class CrmProductService {
    ...
    public Page<Record> queryPage(BasePageRequest<CrmProduct> basePageRequest) {
        return Db.paginate(basePageRequest.getPage(), basePageRequest.getLimit(),
                Db.getSqlPara("crm.product.getProductPageList"));
    }
    ...
    public List<Record> information(Integer id) {
        ...
        field.set("产品名称", record.getStr("name")).set("产品类别", record.getStr("category_name"))
                .set("产品编码", record.getStr("num")).set("标准价格", record.getStr("price"))
                .set("产品描述", record.getStr("description"));
        List<Record> recordList = Db.find("select name,value from 72crm_admin_fieldv where batch_id = ?",
                record.getStr("batch_id"));
    ...
    public R updateStatus(String ids, Integer status) {
        ...
        StringBuilder sqlfield = new StringBuilder(
                "update 72crm_admin_fieldv set value = '" + a + "' where name = '产品状态' and batch_id in ( ");
    ...
    // public List<Record> queryField(Integer productId) {
    // List<Record> fieldList = new ArrayList<>();
    // Record record = Db.findFirst("select * from productview where product_id =
    // ?", productId);
    // String[] settingArr = new String[]{};
    // fieldUtil.getFixedField(fieldList, "name", "产品名称", record.getStr("name"),
    // "text", settingArr, 1);
    // List<Integer> list = crmProductCategoryService.queryId(null,
    // record.getInt("category_id"));
    // Integer[] categoryIds = new Integer[list.size()];
    // categoryIds = list.toArray(categoryIds);
    // fieldUtil.getFixedField(fieldList, "categoryId", "产品分类", categoryIds,
    // "category", settingArr, 1);
    // fieldUtil.getFixedField(fieldList, "num", "产品编码", record.getStr("num"),
    // "number", settingArr, 1);
    // fieldUtil.getFixedField(fieldList, "price", "价格", record.getStr("price"),
    // "floatnumber", settingArr, 1);
    // fieldUtil.getFixedField(fieldList, "description", "产品描述",
    // record.getStr("description"), "text", settingArr, 0);
        ...
        try {
            ...
            if (read.size() > 1) {
                ...
                for (int i = 1; i < read.size(); i++) {
                    ...
                    Integer categoryId = Db.queryInt(
                            "select category_id from 72crm_crm_product_category where name = ?",
                            productList.get(kv.getInt("产品类型(*)")));
                    if (categoryId == null) {
                        return R.error("第" + errNum + 1 + "行填写的产品类型不存在");
                    }
                    if (0 == number) {
                        object.fluentPut("entity",
                                new JSONObject().fluentPut("name", productName)
                        ...
                        Record product = Db.findFirst(
                                "select product_id,batch_id from 72crm_crm_product where name = ?", productName);
                        object.fluentPut("entity",
                                new JSONObject().fluentPut("product_id", product.getInt("product_id"))
                    ...
                    for (Record record : recordList) {
                        record.set("value",
                                productList
                                        .get(kv.getInt(record.getStr("name")) != null ? kv.getInt(record.getStr("name"))
    ...
    public R queryByStatus(BasePageRequest<CrmProduct> basePageRequest) {
        ...
        jsonObject.fluentPut("status",
                new JSONObject().fluentPut("name", "status").fluentPut("condition", "is").fluentPut("value", "1"));
