...
public class CrmProductController extends Controller {
    ...
    /**
     * 产品上禁用 status 0:禁用 1：启用（默认除了0之外其他都是启用）
     *
     * @author zxy
     */
    ...
    private void export(List<Record> recordList) throws IOException {
        ...
        for (Record record : recordList) {
            list.add(record.remove("batch_id", "status", "unit", "category_id", "product_id", "owner_user_id",
                    "create_user_id", "field_batch_id").getColumns());
        }
    ...
    public void downloadExcel() {
        ...
        recordList.removeIf(
                record -> "file".equals(record.getStr("formType")) || "checkbox".equals(record.getStr("formType"))
                        || "user".equals(record.getStr("formType")) || "structure".equals(record.getStr("formType")));
