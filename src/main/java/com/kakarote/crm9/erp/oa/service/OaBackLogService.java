...
public class OaBackLogService {
    ...
    public R backLogNum() {
        ...
        Integer logNum = Db.queryInt(Db.getSql("oa.backLog.queryLogNum"), userId, userId,
                BaseUtil.getUser().getDeptId());
        Integer examineNum = Db.queryInt(Db.getSql("oa.backLog.queryExamineNum"), userId);
        return R.ok().put("data", Kv.by("eventNum", eventNum).set("taskNum", taskNum)
                .set("announcementNum", announcementNum).set("logNum", logNum).set("examineNum", examineNum));
    }
}
