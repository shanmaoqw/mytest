...
public class AdminFieldService {
    ...
    public List<Record> queryAddField(Integer label) {
        ...
        if (label == 2) {
            ...
            fieldList.add(new Record().set("field_name", "type_id").set("name", "商机状态组").set("value", "")
                    .set("form_type", "business_type").set("setting", new String[0]).set("is_null", 1)
                    .set("field_type", 1));
            fieldList.add(new Record().set("field_name", "status_id").set("name", "商机阶段").set("value", "")
                    .set("form_type", "business_status").set("setting", new String[0]).set("is_null", 1)
                    .set("field_type", 1));
        } else if (label == 4) {
            Record map = new Record();
            fieldList.add(
                    map.set("field_name", "address").set("name", "所在区域").set("form_type", "address").set("is_null", 0));
        ...
        if (label == 5 || label == 6) {
            ...
            fieldList.add(record.set("field_name", "product").set("name", "产品")
                    .set("value", Kv.by("discount_rate", "").set("product", new ArrayList<>()).set("total_price", ""))
                    .set("formType", "product").set("setting", new String[] {}).set("is_null", 0).set("field_type", 1));
        }
        return fieldList;
    }
    ...
    public List<Record> queryUpdateField(Integer label, Record record) {
        ...
        recordList.forEach(field -> {
            if (field.getInt("type") == 8) {
                field.set("value", Db.find("select * from 72crm_admin_file where batch_id = ?",
                        StrUtil.isNotEmpty(field.getStr("value")) ? field.getStr("value") : ""));
            }
        });
    ...
    public R save(JSONObject jsonObject) {
        ...
        Map<String, List<AdminField>> collect = adminFields.stream()
                .map(adminField -> TypeUtils.castToJavaBean(adminField, AdminField.class))
                .collect(Collectors.groupingBy(AdminField::getName));
        ...
        if (categoryId != null
                && Db.queryInt("select ifnull(is_sys,0) from 72crm_oa_examine_category where category_id = ?",
                        categoryId) == 1) {
        ...
        if (arr.size() > 0) {
            SqlPara sql = Db.getSqlPara("admin.field.deleteByChooseId",
                    Kv.by("ids", arr).set("label", label).set("categoryId", categoryId));
        ...
        for (int i = 0; i < adminFields.size(); i++) {
            ...
            if (entity.getFieldId() != null) {
                ...
                if (entity.getFieldType() == 0) {
                    ...
                    Db.update("update 72crm_admin_field_sort set name = ? where field_id = ?", entity.getName(),
                            entity.getFieldId());
                }
            } else {
                entity.save();
    ...
    public R verify(Kv kv) {
        Integer number = 0;
        String errorMsg = "参数校验错误";
        String fieldName = kv.getStr("fieldName");
        if ("0".equals(kv.get("fieldType"))) {
            ...
            if (!paramsUtil.isValid(fieldName)) {
                return R.error("参数包含非法字段");
            }
            number = Db.queryInt(
                    "select count(*) from 72crm_crm_" + tableName + " where " + fieldName + " = ? and "
                            + primaryKey + " != ?",
                    kv.getStr("val"), kv.getStr("id") != null ? Integer.valueOf(kv.getStr("id")) : 0);
        }
        return number > 0 ? R.error(errorMsg).put("error", fieldName + "：参数唯一") : R.ok();
    }
    ...
    public synchronized void createView(Integer label) {
        List<Record> fieldNameList = Db.find(
                "select name,type from 72crm_admin_field WHERE label=? and field_type = 0 ORDER BY sorting asc", label);
        ...
        case 1:
            ...
            break;
        case 2:
            filedCreate = String.format(Db.getSql("admin.field.fieldcustomerview"), sql, userJoin.append(deptJoin),
                    label);
            create = Db.getSql("admin.field.customerview");
            break;
        case 3:
            filedCreate = String.format(Db.getSql("admin.field.fieldcontactsview"), sql, userJoin.append(deptJoin),
                    label);
            create = Db.getSql("admin.field.contactsview");
            break;
        case 4:
            filedCreate = String.format(Db.getSql("admin.field.fieldproductview"), sql, userJoin.append(deptJoin),
                    label);
            create = Db.getSql("admin.field.productview");
            break;
        case 5:
            filedCreate = String.format(Db.getSql("admin.field.fieldbusinessview"), sql, userJoin.append(deptJoin),
                    label);
            create = Db.getSql("admin.field.businessview");
            break;
        case 6:
            filedCreate = String.format(Db.getSql("admin.field.fieldcontractview"), sql, userJoin.append(deptJoin),
                    label);
            create = Db.getSql("admin.field.contractview");
            break;
        case 7:
            filedCreate = String.format(Db.getSql("admin.field.fieldreceivablesview"), sql, userJoin.append(deptJoin),
                    label);
    ...
    public List<Record> queryFieldsByBatchId(String batchId, String... name) {
        ...
        return Db.find(AdminField.dao.getSqlPara("admin.field.queryFieldsByBatchId",
                Kv.by("batchId", batchId).set("names", name)));
    }
    ...
    public List<Record> queryByBatchId(String batchId, Integer label) {
        ...
        List<Record> recordList = Db.find(AdminField.dao.getSqlPara("admin.field.queryFieldsByBatchId",
                Kv.by("batchId", batchId).set("label", label)));
        recordList.forEach(record -> {
            if (record.getInt("type") == 10) {
                if (StrUtil.isNotEmpty(record.getStr("value"))) {
                    List<Record> userList = Db.find("select user_id,realname from 72crm_admin_user where user_id in ("
                            + record.getStr("value") + ")");
                ...
                if (StrUtil.isNotEmpty(record.getStr("value"))) {
                    List<Record> deptList = Db.find("select dept_id,name from 72crm_admin_dept where dept_id in ("
                            + record.getStr("value") + ")");
    ...
    private void recordValueToArray(Record record) {
        record.set("default_value",
                StrUtil.isNotEmpty(record.get("default_value")) ? record.getStr("default_value").split(",")
                        : new String[] {});
        record.set("value",
                StrUtil.isNotEmpty(record.getStr("value")) ? record.getStr("value").split(",") : new String[] {});
    }

    /**
     * @author wyq 查询fieldType为0的字段
     */
    ...
    public List<Record> list(String label, String categoryId) {
        List<Record> recordList = Db
                .find(Db.getSqlPara("admin.field.list", Kv.by("label", label).set("categoryId", categoryId)));
        recordToFormType(recordList);
        if (categoryId == null) {
            return recordList;
        }
    ...
    public R setFelidStyle(Kv kv) {
        ...
        AdminFieldStyle adminFleldStyle = AdminFieldStyle.dao.findFirst(
                AdminFieldStyle.dao.getSql("admin.field.queryFieldStyle"), type, kv.getStr("field"),
                BaseUtil.getUser().getUserId());
    ...
    public List<AdminFieldStyle> queryFieldStyle(String type) {
        ...
        return AdminFieldStyle.dao.find("select * from 72crm_admin_field_style where type = ? and user_id = ?", type,
                userId);
    }

    /**
     * @author wyq 查询客户管理列表页字段
     */
    @Before(Tx.class)
    public List<Record> queryListHead(AdminFieldSort adminFieldSort) {
        // 查看userid是否存在于顺序表，没有则插入
        Long userId = BaseUtil.getUser().getUserId();
        Integer number = Db.queryInt("select count(*) from 72crm_admin_field_sort where user_id = ? and label = ?",
                userId, adminFieldSort.getLabel());
        if (0 == number) {
            ...
            fieldUtil.add("updateTime", "更新时间").add("createTime", "创建时间").add("ownerUserName", "负责人")
        ...
        return Db.findByCache("field", "listHead:" + adminFieldSort.getLabel() + userId,
                Db.getSql("admin.field.queryListHead"), adminFieldSort.getLabel(), userId);
    }

    /**
     * @author wyq 查询字段排序隐藏设置
     */
    @Before(Tx.class)
    public R queryFieldConfig(AdminFieldSort adminFieldSort) {
        ...
        for (Record record : fieldList) {
            ...
            Integer number = Db.queryInt(
                    "select count(*) as number from 72crm_admin_field_sort where user_id = ? and label = ? and field_name = ?",
                    userId, adminFieldSort.getLabel(), fieldName);
            if (number.equals(0)) {
                AdminFieldSort newField = new AdminFieldSort();
                newField.setFieldName(fieldName).setName(fieldName).setLabel(adminFieldSort.getLabel()).setIsHide(1)
                        .setUserId(userId).setSort(1);
                newField.save();
            }
        }
        List<Record> noHideList = Db.find(Db.getSql("admin.field.queryFieldConfig"), 0, adminFieldSort.getLabel(),
                userId);
        List<Record> hideList = Db.find(Db.getSql("admin.field.queryFieldConfig"), 1, adminFieldSort.getLabel(),
                userId);
        return R.ok().put("data", Kv.by("value", noHideList).set("hide_value", hideList));
    }

    /**
     * @author wyq 客户管理列表页字段是否隐藏
     */
    @Before(Tx.class)
    public R fieldConfig(AdminFieldSort adminFieldSort) {
        ...
        if (null != adminFieldSort.getHideIds()) {
            ...
            Db.update(Db.getSqlPara("admin.field.isHide",
                    Kv.by("ids", hideIdsArr).set("label", adminFieldSort.getLabel()).set("userId", userId)));
        }
