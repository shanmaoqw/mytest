...
public class AdminUserService {
    ...
    public R deleteUserByIds(String userIds){
        ...
        for (Integer id : TagUtil.toSet(userIds)) {
            //删除账号
            Db.delete("delete from 72crm_admin_user where user_id = ?", id);
            //删除账号角色
            Db.delete("delete from 72crm_admin_user_role where user_id = ?",id);
        }
        return R.ok().put("data","账号已删除");
    }
}
