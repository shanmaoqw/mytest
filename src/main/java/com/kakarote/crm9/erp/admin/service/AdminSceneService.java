...
import com.kakarote.crm9.erp.crm.common.CrmViewEnum;
...
public class AdminSceneService {
	...
	public R queryField(Integer label) {
		...
		if (1 == label) {
			fieldUtil.add("leads_name", "线索名称", "text", settingArr).add("telephone", "电话", "text", settingArr)
					.add("mobile", "手机", "mobile", settingArr).add("address", "地址", "text", settingArr)
					.add("next_time", "下次联系时间", "datetime", settingArr).add("remark", "备注", "text", settingArr)
					.add("owner_user_id", "负责人", "user", settingArr).add("create_user_id", "创建人", "user", settingArr)
					.add("update_time", "更新时间", "datetime", settingArr)
					.add("create_time", "创建时间", "datetime", settingArr);
		} else if (2 == label) {
			String[] dealStatusArr = new String[] { "未成交", "已成交" };
			fieldUtil.add("customer_name", "客户名称", "text", settingArr).add("mobile", "手机", "text", settingArr)
					.add("telephone", "电话", "text", settingArr).add("website", "网址", "text", settingArr)
					.add("next_time", "下次联系时间", "datetime", settingArr).add("remark", "备注", "text", settingArr)
					// .add("detail_address", "详细地址", "text", settingArr)
					.add("map_address", "地区", "map_address", settingArr)
					.add("deal_status", "成交状态", "select", dealStatusArr).add("owner_user_id", "负责人", "user", settingArr)
					.add("create_user_id", "创建人", "user", settingArr).add("update_time", "更新时间", "datetime", settingArr)
					.add("create_time", "创建时间", "datetime", settingArr);
		} else if (3 == label) {
			fieldUtil.add("name", "姓名", "text", settingArr).add("customer_name", "客户名称", "customer", settingArr)
					.add("mobile", "手机", "mobile", settingArr).add("telephone", "电话", "text", settingArr)
					.add("email", "电子邮箱", "email", settingArr).add("post", "职务", "text", settingArr)
					.add("address", "地址", "text", settingArr).add("next_time", "下次联系时间", "datetime", settingArr)
					.add("remark", "备注", "text", settingArr).add("owner_user_id", "负责人", "user", settingArr)
					.add("create_user_id", "创建人", "user", settingArr).add("update_time", "更新时间", "datetime", settingArr)
					.add("create_time", "创建时间", "datetime", settingArr);
		} else if (4 == label) {
			fieldUtil.add("name", "产品名称", "text", settingArr).add("category_id", "产品分类", "category", settingArr)
					.add("num", "产品编码", "number", settingArr).add("price", "价格", "floatnumber", settingArr)
					.add("map_address", "地区", "map_address", settingArr).add("description", "产品描述", "text", settingArr)
					.add("owner_user_id", "负责人", "user", settingArr).add("create_user_id", "创建人", "user", settingArr)
					.add("update_time", "更新时间", "datetime", settingArr)
					.add("create_time", "创建时间", "datetime", settingArr);
		} else if (5 == label) {
			fieldUtil.add("business_name", "商机名称", "text", settingArr)
					...
					.add("type_id", "商机状态组", "business_type",
							crmBusinessService.queryBusinessStatusOptions("condition"))
					.add("money", "商机金额", "floatnumber", settingArr).add("deal_date", "预计成交日期", "datetime", settingArr)
					.add("remark", "备注", "text", settingArr).add("product", "产品", "product", settingArr)
					.add("owner_user_id", "负责人", "user", settingArr).add("create_user_id", "创建人", "user", settingArr)
					.add("update_time", "更新时间", "datetime", settingArr)
					.add("create_time", "创建时间", "datetime", settingArr);
		} else if (6 == label) {
			...
			fieldUtil.add("num", "合同编号", "number", settingArr).add("name", "合同名称", "text", settingArr)
					.add("check_status", "审核状态", "checkStatus", checkList)
					.add("customer_name", "客户名称", "customer", settingArr)
					.add("business_name", "商机名称", "business", settingArr).add("order_date", "下单时间", "date", settingArr)
					.add("money", "合同金额", "floatnumber", settingArr).add("start_time", "合同开始时间", "datetime", settingArr)
					.add("end_time", "合同结束时间", "datetime", settingArr)
					.add("contacts_id", "客户签约人", "contacts", settingArr)
					.add("company_user_id", "公司签约人", "user", settingArr).add("remark", "备注", "number", settingArr)
					.add("product", "产品", "product", settingArr).add("owner_user_id", "负责人", "user", settingArr)
					.add("create_user_id", "创建人", "user", settingArr).add("update_time", "更新时间", "datetime", settingArr)
					.add("create_time", "创建时间", "datetime", settingArr);
		} else if (7 == label) {
			List<Map<String, Object>> checkList = new ArrayList<>();
			...
			checkList.add(new JSONObject().fluentPut("name", "审核未通过").fluentPut("value", 3));
			checkList.add(new JSONObject().fluentPut("name", "已撤回").fluentPut("value", 4));
			fieldUtil.add("number", "回款编号", "number", settingArr).add("check_status", "审核状态", "checkStatus", checkList)
					.add("customer_name", "客户名称", "customer", settingArr)
					.add("contract_num", "合同编号", "contract", settingArr).add("return_time", "回款日期", "date", settingArr)
					.add("money", "回款金额", "floatnumber", settingArr).add("remark", "备注", "textarea", settingArr)
					.add("owner_user_id", "负责人", "user", settingArr).add("create_user_id", "创建人", "user", settingArr)
					.add("update_time", "更新时间", "datetime", settingArr)
					.add("create_time", "创建时间", "datetime", settingArr);
		} else {
	...
	public R updateScene(AdminScene adminScene) {
		...
		if (1 == adminScene.getIsDefault()) {
			Db.update("update 72crm_admin_scene_default set scene_id = ? where user_id = ? and type = ?",
					adminScene.getSceneId(), userId, oldAdminScene.getType());
		}
		adminScene.setUserId(userId).setType(oldAdminScene.getType()).setSort(oldAdminScene.getSort())
				.setIsSystem(oldAdminScene.getIsSystem()).setUpdateTime(DateUtil.date());
	...
	public R setDefaultScene(Integer sceneId) {
		...
		Db.delete("delete from 72crm_admin_scene_default where user_id = ? and type = ?", userId,
				oldAdminScene.getType());
		AdminSceneDefault adminSceneDefault = new AdminSceneDefault();
		return adminSceneDefault.setSceneId(sceneId).setType(oldAdminScene.getType()).setUserId(userId).save() ? R.ok()
				: R.error();
	}
	...
	public R queryScene(Integer type) {
		...
		if (number.equals(0)) {
			...
			systemScene.setUserId(userId).setSort(0).setData("").setIsHide(0).setIsSystem(1)
					.setCreateTime(DateUtil.date()).setType(type);
			JSONObject ownerObject = new JSONObject();
			ownerObject.fluentPut("owner_user_id", new JSONObject().fluentPut("name", "owner_user_id")
					.fluentPut("condition", "is").fluentPut("value", userId));
			JSONObject subOwnerObject = new JSONObject();
			subOwnerObject.fluentPut("owner_user_id",
					new JSONObject().fluentPut("name", "owner_user_id").fluentPut("condition", "in").fluentPut("value",
							getSubUserId(userId.intValue(), BaseConstant.AUTH_DATA_RECURSION_NUM).substring(1)));
			if (1 == type) {
				systemScene
						.setName(
								"全部线索")
						.setData(
								new JSONObject()
										.fluentPut("is_transform",
												new JSONObject().fluentPut("name", "is_transform")
														.fluentPut("condition", "is").fluentPut("value", 0))
										.toString())
						.save();
				ownerObject
						.fluentPut("owner_user_id",
								new JSONObject().fluentPut("name", "owner_user_id").fluentPut("condition", "is")
										.fluentPut("value", userId))
						.fluentPut("is_transform", new JSONObject().fluentPut("name", "is_transform")
								.fluentPut("condition", "is").fluentPut("value", 0));
				systemScene.setSceneId(null).setName("我负责的线索").setData(ownerObject.toString()).save();
				subOwnerObject
						.fluentPut("owner_user_id",
								new JSONObject().fluentPut("name", "owner_user_id").fluentPut("condition", "in")
										.fluentPut("value",
												getSubUserId(userId.intValue(), BaseConstant.AUTH_DATA_RECURSION_NUM)
														.substring(1)))
						.fluentPut("is_transform", new JSONObject().fluentPut("name", "is_transform")
								.fluentPut("condition", "is").fluentPut("value", 0));
				systemScene.setSceneId(null).setName("下属负责的线索").setData(subOwnerObject.toString()).save();
				JSONObject jsonObject = new JSONObject();
				jsonObject.fluentPut("is_transform", new JSONObject().fluentPut("name", "is_transform")
						.fluentPut("condition", "is").fluentPut("value", "1"));
				...
				JSONObject jsonObject = new JSONObject();
				jsonObject.fluentPut("ro_user_id", new JSONObject().fluentPut("name", "ro_user_id")
						.fluentPut("condition", "takePart").fluentPut("value", userId));
				...
				systemScene
						.setName(
								"启用的产品")
						.setData(new JSONObject().fluentPut("产品状态", new JSONObject().fluentPut("name", "产品状态")
								.fluentPut("condition", "is").fluentPut("value", "启用")).toString())
						.save();
				JSONObject jsonObject = new JSONObject();
				jsonObject.fluentPut("产品状态", new JSONObject().fluentPut("name", "产品状态").fluentPut("condition", "is")
						.fluentPut("value", "禁用"));
				systemScene.setSceneId(null).setName("禁用的产品").setData(jsonObject.toString()).save();
			} else if (5 == type) {
	...
	public R sceneConfig(AdminScene adminScene) {
		...
		if (null != adminScene.getHideIds()) {
			...
			Db.update(Db.getSqlPara("admin.scene.isHide",
					Kv.by("ids", hideIdsArr).set("type", adminScene.getType()).set("userId", userId)));
		}
		return R.ok();
	}

	/**
	 * 方法描述: 通过类型查询不同条件的客户分页信息</br>
	 * 初始作者: WenBin<br/>
	 * 创建日期: 2019年8月16日-下午3:55:43<br/>
	 * 开始版本: 2.0.0<br/>
	 * =================================================<br/>
	 * 修改记录：<br/>
	 * 修改作者 日期 修改内容<br/>
	 ...
	 * @return R
	 */
	public R filterConditionAndGetPageList(@SuppressWarnings("rawtypes") BasePageRequest basePageRequest) {
		...
		if (sceneId == null && jsonObject.getInteger("type") == 1) {
			data = new JSONObject().fluentPut("is_transform", new JSONObject().fluentPut("name", "is_transform")
					.fluentPut("condition", "is").fluentPut("value", "禁用"));
		}
		if (sceneId == null && jsonObject.getInteger("type") == 4) {
			data = new JSONObject().fluentPut("产品状态",
					new JSONObject().fluentPut("name", "产品状态").fluentPut("condition", "is").fluentPut("value", "启用"));
		}
	 ...
	 * @return R
	 */
	public R getCrmPageList(@SuppressWarnings("rawtypes") BasePageRequest basePageRequest) {
		Integer type = basePageRequest.getJsonObject().getInteger("type");
		String viewName;
		switch (type) {
		case 1:
			viewName = CrmViewEnum.LEADS_VIEW.getSign();
			break;
		case 2:
			viewName = CrmViewEnum.CUSTOMER_VIEW.getSign();
			break;
		case 3:
			viewName = CrmViewEnum.CONTACTS_VIEW.getSign();
			break;
		case 4:
			viewName = CrmViewEnum.PRODUCT_VIEW.getSign();
			break;
		case 5:
			viewName = CrmViewEnum.BUSINESS_VIEW.getSign();
			break;
		case 6:
			viewName = CrmViewEnum.CONTRACT_VIEW.getSign();
			break;
		case 7:
			viewName = CrmViewEnum.RECEIVABLES_VIEW.getSign();
			break;
		case 8:
			viewName = CrmViewEnum.CUSTOMER_VIEW.getSign();
			break;
		...
		for (JSONObject jsonObject : jsonObjectList) {
			...
			if (StrUtil.isNotEmpty(jsonObject.getString("start"))
					&& !paramsUtil.isValid(jsonObject.getString("start"))) {
				return R.error("参数包含非法字段");
			}
			...
			if (StrUtil.isNotEmpty(value) || StrUtil.isNotEmpty(jsonObject.getString("start"))
					|| StrUtil.isNotEmpty(jsonObject.getString("end"))) {
				if ("takePart".equals(condition)) {
					conditions.append(" and (ro_user_id like '%,").append(value).append(",%' or rw_user_id like '%,")
							.append(value).append(",%')");
				} else {
					...
					if ("datetime".equals(formType)) {
						conditions.append(" between '").append(jsonObject.getString("start")).append("' and '")
								.append(jsonObject.getString("end")).append("'");
					}
					if ("date".equals(formType)) {
						conditions.append(" between '").append(jsonObject.getString("startDate")).append("' and '")
								.append(jsonObject.getString("endDate")).append("'");
					}
				}
			}
		}
		...
		if (StrUtil.isNotEmpty(search)) {
			...
			if (type == 1) {
				...
				conditions.append(" and (name like '%").append(search).append("%' or telephone like '%").append(search)
						.append("%' or mobile like '%").append(search).append("%')");
		...
		if (!type.equals(8) && !type.equals(4) && !BaseConstant.SUPER_ADMIN_USER_ID.equals(userId)) {
			...
			if (longs != null && longs.size() > 0) {
				...
				if (type.equals(2) || type.equals(6) || type.equals(5)) {
					conditions.append(" or ro_user_id like CONCAT('%,','").append(userId).append("',',%')")
							.append(" or rw_user_id like CONCAT('%,','").append(userId).append("',',%')");
				}
			}
		}
		...
		if (2 == type || 8 == type) {
			Integer configType = Db
					.queryInt("select status from 72crm_admin_config where name = 'customerPoolSetting'");
			if (1 == configType && 2 == type) {
				return R.ok().put("data", Db.paginate(basePageRequest.getPage(), basePageRequest.getLimit(),
						Db.getSql("admin.scene.getCustomerPageList"), conditions.toString()));
			} else {
				return R.ok().put("data",
						Db.paginate(basePageRequest.getPage(), basePageRequest.getLimit(),
								"select *,(select count(*) from 72crm_crm_business as a where a.customer_id = "
										+ viewName + ".customer_id) as business_count",
								conditions.toString()));
			}

		} else if (6 == type) {
			Record totalMoney = Db
					.findFirst("select SUM(money) as contractMoney,GROUP_CONCAT(contract_id) as contractIds "
							+ conditions.toString());
			Page<Record> page = Db.paginate(basePageRequest.getPage(), basePageRequest.getLimit(),
					"select *,IFNULL((select SUM(a.money) from 72crm_crm_receivables as a where a.contract_id = contractview.contract_id),0) as receivedMoney",
					conditions.toString());

			String receivedMoney = Db.queryStr("select SUM(money) from 72crm_crm_receivables where receivables_id in ("
					+ totalMoney.getStr("contractIds") + ")");
			JSONObject jsonObject = JSONObject.parseObject(Json.getJson().toJson(page), JSONObject.class);
			return R.ok().put("data",
					jsonObject.fluentPut("money", new JSONObject()
							.fluentPut("contractMoney",
									totalMoney.getStr("contractMoney") != null ? totalMoney.getStr("contractMoney")
											: "0")
							.fluentPut("receivedMoney", receivedMoney != null ? receivedMoney : "0")));
		}
		Page<Record> recordPage = Db.paginate(basePageRequest.getPage(), basePageRequest.getLimit(), "select *",
				conditions.toString());
		if (type == 5) {
	...
	public void setBusinessStatus(List<Record> list) {
		list.forEach(record -> {
			if (record.getInt("is_end") == 0) {
				Integer sortNum = Db.queryInt("select order_num from 72crm_crm_business_status where status_id = ?",
						record.getInt("status_id"));
				int totalStatsNum = Db.queryInt("select count(*) from 72crm_crm_business_status where type_id = ?",
						record.getInt("type_id")) + 1;
